...
export function CategoriesView() {
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <section className="rounded-3xl bg-emerald-950 p-6 text-white shadow-xl shadow-emerald-950/10 md:p-8">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_420px] lg:items-end">
          <div>
            <h1 className="text-4xl font-black tracking-tight md:text-5xl">Categories</h1>
      ...
      <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {filtered.map((category, index) => {
          const count = products.filter((p) => p.category === category).length || 12 + index * 3;
          return (
            <Link key={category} href={`/?categorie=${encodeURIComponent(category)}`} className="group overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-sm shadow-emerald-950/5 transition hover:-translate-y-1 hover:shadow-xl hover:shadow-emerald-950/10">
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-semibold text-slate-500">{count} produits</p>
                  </div>
                  <span className="grid size-10 place-items-center rounded-xl bg-emerald-50 text-emerald-800 transition group-hover:bg-emerald-800 group-hover:text-white"><ArrowRight className="size-4" /></span>
                </div>
                <div className="mt-5 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-emerald-700" style={{ width: `${Math.min(95, 35 + count)}%` }} /></div>
              </div>
            </Link>
          );
        })}
